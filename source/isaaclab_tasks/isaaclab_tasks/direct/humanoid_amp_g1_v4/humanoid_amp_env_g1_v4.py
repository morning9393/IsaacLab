# Copyright (c) 2022et2025, The Isaac Lab Project Developers.
# All rights reserved.
#
...
from .humanoid_amp_env_cfg_g1_v4 import HumanoidAmpEnvCfg
from .motions import MotionLoaderG1
...
class HumanoidAmpEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: HumanoidAmpEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # load motion
        self._motion_loader = MotionLoaderG1(motion_file=self.cfg.motion_file, device=self.device)
        ...
        # import ipdb; ipdb.set_trace()
        self.motion_dof_indexes, self.motion_dof_select_robot_indexes = self._motion_loader.get_dof_index(self.robot.data.joint_names)
        # import ipdb; ipdb.set_trace()
        self.motion_ref_body_index = self._motion_loader.get_body_index([self.cfg.reference_body_motion])[0]
        self.motion_key_body_indexes = self._motion_loader.get_body_index(motion_key_body_names)
        # import ipdb; ipdb.set_trace() 
    ...
    def _get_observations(self) -> dict:
        ...
        obs = compute_obs(
            self.robot.data.joint_pos[:, self.motion_dof_select_robot_indexes],
            self.robot.data.joint_vel[:, self.motion_dof_select_robot_indexes],
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # import ipdb; ipdb.set_trace()
        # test_root_state = torch.zeros_like(root_state)
        self.robot.write_root_link_pose_to_sim(root_state[:, :7], env_ids)
        self.robot.write_root_com_velocity_to_sim(root_state[:, 7:], env_ids)
        # import ipdb; ipdb.set_trace()
        # self.robot.write_joint_state_to_sim(joint_pos, joint_vel, None, env_ids)
        self.robot.write_joint_state_to_sim(joint_pos, joint_vel, joint_ids=self.motion_dof_select_robot_indexes, env_ids=env_ids)
    # reset strategies
    ...
    def _reset_strategy_random(
        ...
        motion_torso_index = self._motion_loader.get_body_index(["torso_link"])[0]
        root_state = self.robot.data.default_root_state[env_ids].clone()
